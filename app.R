#
# Dashboard for "Infraestructura verde, gris y azul"
#
  ...
  function() {
    leaflet() |>
      ...
      addLayersControl(
        ...
        overlayGroups = c("Infraestructura verde, gris y azul", "Límite cantonal"),
        position = "bottomleft",
...
create_barplot <-
  function(canton) {
    # Filter
    canton_data <- infrastructure_cantons |>
      filter(Cantón == canton) %>%
      gather(key = "category", value = "area",-Cantón)
    ...
    barplot_infrastructure_ggplot2 <-
      ...
      coord_flip() +
      labs(
        title = paste0("Infraestructura del cantón de ", canton),
        x = element_blank(),
        y = "Área (ha)"
      ) +
      theme_classic(
...
DSN_INFRASTRUCTURE_CANTONS <- "data/infraestructura-cantones.xlsx"

# Cantons
cantons <- st_read(dsn = DSN_CANTONS, quiet = TRUE)

# Cantons and infrastructure
infrastructure_cantons <- read_excel(DSN_INFRASTRUCTURE_CANTONS, sheet = 1)
...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    fluidRow(h4("Haga clic en el mapa para mostrar los datos de un cantón en el gráfico")),
...
server <- function(input, output, session) {
  ...
  output$barplot_infrastructure <- renderPlotly({
    create_barplot(selected_canton())
  })  
